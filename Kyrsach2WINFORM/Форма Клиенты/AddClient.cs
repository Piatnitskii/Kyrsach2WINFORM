...
namespace Kyrsach2WINFORM
{
    public partial class AddClient : Form
    {
        ...
        {
            string CMD = $"Select * FROM Client WHERE Phone = '{NumberPhone}' AND Name = '{Name}' AND Surname = '{Surname}' AND Patronymic = '{Patronymic}'";
        ...
        private void AddClient_Click(object sender, EventArgs e)
        {
            ...
            string CMD = $"INSERT INTO Client (Name, Surname, Patronymic, Phone) VALUES ('{Name}', '{Surname}', '{Patronymic}',  '{NumberPhone}');";
            try
            {
                DialogResult dialogResult = MessageBox.Show("Добавить клиента?", "Предупреждение", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (dialogResult == DialogResult.Yes)
                {
                    if (textBox3.Text.Trim() == "")
                    {
                        ...
                        if (dialogResultTwo == DialogResult.Yes)
                            CMD = $"INSERT INTO Client (Name, Surname,Phone) VALUES ('{Name}', '{Surname}','{NumberPhone}');"; // тут меняем строку запроса, с отчеством
                        else
                            return;
                    }
                    ...
                    {
                        MessageBox.Show("Данный клиент уже существует в базе", "Ошибка операции", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        Clear();    // Очистка
                        return;
                    }
